(function (w, d) {
    var prot = /^((https|chrome-extension):)/i.test(w.location.protocol)?"https":"http", u = prot + "://share.pluso.ru", uid = "6790990661057659839", tags = "getElementsByTagName", h = d[tags]("body")[0], b = d.documentElement, db = d.body, compat = 0;
    ...
    function osize(opt) {return opt.small == 1?"small":opt.medium == 1?"medium":"big"}
    ...
    var iterate = function (obj, callback) {for (var key in obj)if (obj.hasOwnProperty(key))callback(key, obj)};
    var elements = d.querySelectorAll?function (className, context) {return (context || d).querySelectorAll("." + className)}:function (className, context) {
        if (! className)return [];
    ...
    var cloned = function (obj) {
        ...
        var clone = {};
        for (var i in obj){
            if (typeof obj[i] == "object")clone[i] = cloned(obj[i]); else clone[i] = obj[i]
        }
        return clone
    };
    var arrayed = function (obj) {return Object.prototype.toString.call(obj) === "[object Array]"};
    ...
    pluso.lang["ru"] = {
        ...
        get     : "РљРЅРѕРїРєРё СѓРІРµР»РёС‡РёРІР°СЋС‚ С‚СЂР°С„РёРє",
        share   : "РџРѕРґРµР»РёС‚РµСЃСЊ СЃ РґСЂСѓР·СЊСЏРјРё!",
        bookmark: "РЎРєРѕРїРёСЂСѓР№С‚Рµ Рё РґРѕР±Р°РІСЊС‚Рµ СЌС‚Сѓ СЃСЃС‹Р»РєСѓ РІ Р—Р°РєР»Р°РґРєРё",
        buy     : "РљСѓРїРёС‚СЊ"
    };
    pluso.lang["ua"] = {
        close   : "Р—Р°РєСЂРёС‚Рё",
        get     : "РћС‚СЂРёРјР°С”С‚Рµ СЃРІРѕС— РєРЅРѕРїРєРё",
        share   : "РџРѕРґС–Р»РёС‚РµСЃСЏ Р· РґСЂСѓР·СЏРјРё!",
        bookmark: "РЎРєРѕРїС–СЋР№С‚Рµ С‚Р° РґРѕРґР°Р№С‚Рµ С†СЋ РїРѕСЃРёР»Р°РЅРЅСЏ РІ Р—Р°РєР»Р°РґРєРё",
        buy     : "РљСѓРїРёС‚Рё"
    };
    pluso.lang["by"] = {
        close   : "Р—Р°С‡С‹РЅС–С†СЊ",
        get     : "РђС‚СЂС‹РјР°РµС†Рµ СЃРІР°Рµ РєРЅРѕРїРєС–",
        share   : "РџР°РґР·СЏР»С–С†РµСЃСЏ Р· СЃСЏР±СЂР°РјС–!",
        bookmark: "РЎРєР°РїС–СЋР№С†Рµ С– РґР°РґР°Р№С†Рµ РіСЌС‚СѓСЋ СЃРїР°СЃС‹Р»РєСѓ Сћ Р—Р°РєР»Р°РґРєС–",
        buy     : "РљСѓРїС–С†СЊ"
    };
    pluso.lang["en"] = {
        close   : "Close",
    ...
    pluso.tree = [["sepcounter", "horizontal", "vertical", "multiline", "line", "small", "big", "medium", "counter", "nocounter", "round", "square"], {
        horizontal: "vertical",
        multiline : "line",
        small     : ["big", "medium"],
        counter   : "nocounter",
    ...
    pluso.advs = false;
    pluso.user = false;
    pluso.offsets = {small: 21, medium: 31, big: 42};
    pluso.tabbed = ["livejournal", "liveinternet", "stumbleupon", "bobrdobr", "evernote", "instapaper", "digg"];
    pluso.defaults = {
        options : {horizontal: 1, line: 1, small: 1, counter: 1, round: 1, theme: "04", sepcounter: 0},
        services: [],
        params  : {
            background : "#eaeaea",
            baloon     : "#eaeaeb",
    ...
    pluso.html = function (elem, debug) {
        ...
        var replacer = function (p) {return p.replace(/\{(\w+)\}/g, function (s, key) {return reps[key] || s})};
        ...
        for (var i = 0, l = ep.services.length, q = l / 2; i < l; i ++)if (typeof pluso.services[ep.services[i]] != "undefined"){
    ...
    pluso.build = function (elem, debug) {
        ...
        more.onmouseout = function () {clearTimeout(more.timer)};
        return elem
    };
    pluso.start = function (el) {
        ...
        (function (i, s, o, g, r, a, m) {
            setTimeout(function () {
                ...
                fcz("collect", "pluso", "", {pixel_id: "176"})
            }, 50);
        })(window, document, "script", prot + "://front.facetz.net/collect_pluso.js", "fcz");
    };
    pluso.share_pinterest = function () {
        js("http://assets.pinterest.com/js/pinmarklet.js?r=" + Math.random() * 99999999)
    };
    pluso.share_pinme = function () {js("http://st.pinme.ru/js/pinbutton.js?r=" + Math.random() * 99999999)};
    pluso.share_readability = function () {js("http://www.readability.com/bookmarklet/save.js?r=" + Math.random() * 99999999)};
    pluso.share_print = function () {window.print()};
    pluso.share_bookmark = function (ep) {
        ...
        } else prompt("РЎРєРѕРїРёСЂСѓР№С‚Рµ Рё РґРѕР±Р°РІСЊС‚Рµ СЌС‚Сѓ СЃСЃС‹Р»РєСѓ РІ Р—Р°РєР»Р°РґРєРё", ep.params.url)
    };
    ...
    pluso.params = function (elem) {return "u=" + encode(elem.pluso.params.url) + (pluso.screenWidth && pluso.screenHeight?"&w=" + pluso.screenWidth + "&h=" + pluso.screenHeight:"") + "&ref=" + encode(d.referrer) + (elem.pluso.params.user > 0?"&user=" + encode(elem.pluso.params.user):"") + "&uid=" + uid + "&k=" + pluso.randomString(16)};
    pluso.share = function (t, elem, menu) {
        var sharer = t.className.substring(6), share = pluso["share_" + sharer], sharelink = pluso.sharelink(sharer, elem, menu);
    ...
    pluso.more = function (xy, elem) {
        if (elem.box){
            ...
            div.onmouseover = function (ev) {if (div.timer)clearTimeout(div.timer)};
        ...
        var z = pluso.offsets[osize(elem.pluso.options)] / 2, x = xy.pageX - z, y = xy.pageY - z, ws = dimensions();
        if (x < 0)x = 10; else if (x + 310 > ws.width)x = ws.width - 350;
        if (y < 0)y = 5; else if (y + 265 > ws.height)y = ws.height - 265;
        div.style.left = x + "px";
        div.style.top = y + "px";
        if (elem.box){
    ...
    pluso.process = function (action, elem, params) {js("+process?act=" + action + "&" + pluso.params(elem) + (params || ""))};
    pluso.setCounter = function (c, url, buy, counters, advType) {
        var el = elements("pluso");
        if (typeof buy != "undefined"){
            for (var i = 0, l = el.length; i < l; i ++)if (el[i].className.split(" ").indexOf("pluso-skip") == - 1 && el[i].getAttribute("data-user") !== null){
        ...
        if (! pluso.advs && advType){
            ...
            pluso.advs = adv;
            if (adv.length){
                var requestSrc = "http://ads.betweendigital.com/adj?s=95614&h=400&w=240", divForScript = document.createElement("div");
                ...
                if (iframeDocument = iframe.contentDocument){
                    ...
                    closeButton.addEventListener("click", function () {divForScript.style.display = "none"});
    ...
    pluso.market = function (items) {
        ...
        for (var i = 0, l = items.length; i < l; i ++){
            var it = items[i];
            html.push('<li><a href="' + it.url + '" target="_blank"><span><img src="//offers.pluso.ru/img/' + it.image + '.png" border="0" alt="" height="150"></span><strong>' + it.title + "</strong><em>" + (it.price * 1).toString() + " СЂСѓР±.<u>РР· СЂСѓРє РІ СЂСѓРєРё</u></em><b>РљСѓРїРёС‚СЊ!</b></a></li>")
        }
